...
#define HPET0_BASE      0x16120000
#define HPET1_BASE      0x16121000
#define HPET2_BASE      0x16122000
#define HPET3_BASE      0x16123000

/*
 * HPET 控制器. XXX 64 位寄存器
 */
typedef struct
{
 ...
 */
#define HPET_CNT_CLK_PERIOD_MASK    (0xFFFFFFFFul<<32)  /* RO bit[63:32] 这个域标示了主计时器的计时频率, 以fs(10^-15s)为单位. */
#define HPET_CNT_CLK_PERIOD_SHIFT   32                  /*               这个值必须大于0, 且小于或等于0x05F5E100(100ns, 即10MHz) */
#define HPET_VENDOR_ID_MASK         (0xFFFFul<<16)      /* RO bit[31:16] */
#define HPET_VENDOR_ID_SHIFT        16
#define HPET_COUNT_SIZE_64          (1ul<<13)           /* RO 主计时器的宽度: 0=32 bits; 1=64 bits */
#define HPET_NUM_TIMERS_MASK        0x0F00ul            /* RO bit[12:8] 定时器的个数-1 */
 ...
 *	如果是电平触发模式:	这位默认是0. 当对应的定时器发生中断, 那么有硬件将其置1.
 *  一旦被置位, 软件往这位写1将会清空这位. 往这位写0, 则无意义.
 *	如果边沿触发模式:	软件将忽略这位. 软件通常往这位写0.
 */
...
/**
 * Main Counter Value Register. XXX 只使用了32位 ?
 */
...
#define HPET_PERIODIC_INT_EN     (1<<3)          /* RW	Rx Timer N type(N为0-2):
...
/*
 * Timer N Comparator Value Register. XXX 只使用了32位 ?
 */
