 ...
 * RTC device
 */
static rtc_priv_t ls2k_RTC =
{
    .hwRTC = (HW_RTC_t *)PHYS_TO_UNCACHED(RTC_BASE),
    .initialized = 0,
};

static rtc_priv_t *pRTC = &ls2k_RTC;
...
static int ls2k_rtc_stop(int index)
{
    ...
    switch (index)
    {
        case 0: ls2k_remove_irq_handler(EXTI1_RTC_0_IRQ); break;
        case 1: ls2k_remove_irq_handler(EXTI1_RTC_1_IRQ); break;
        case 2: ls2k_remove_irq_handler(EXTI2_RTC_2_IRQ); break;
        default: return -1;
    }
#endif
...
static int ls2k_toy_stop(int index)
{
    ...
    switch (index)
    {
        case 0: ls2k_remove_irq_handler(EXTI2_TOY_0_IRQ); break;
        case 1: ls2k_remove_irq_handler(EXTI2_TOY_1_IRQ); break;
        case 2: ls2k_remove_irq_handler(EXTI2_TOY_2_IRQ); break;
        default: return -1;
    }
#endif
 ...
 * 中断处理程序
 *
 *   使用共享中断时, 如何区分当前发生的是哪个中断?
 *
 */
static void ls2k_rtc_shared_isr(int vector, void *arg)
{
    int device, index;
    int stop = 0;
...
static int ls2k_rtc_install_isr(int device, int index)
{
    ...
    switch (device)
    {
        case LS2K_RTC:
#if !USE_EXTINT
            irqnum = INTC1_RTC_IRQ;

#else
            switch (index)
            {
                case 0: irqnum = EXTI1_RTC_0_IRQ; break;
                case 1: irqnum = EXTI1_RTC_1_IRQ; break;
                case 2: irqnum = EXTI2_RTC_2_IRQ; break;
                default: return -1;
            }
        ...
        case LS2K_TOY:
#if !USE_EXTINT
            irqnum = INTC1_TOY_IRQ;

#else
            switch (index)
            {
                case 0: irqnum = EXTI2_TOY_0_IRQ; break;
                case 1: irqnum = EXTI2_TOY_1_IRQ; break;
                case 2: irqnum = EXTI2_TOY_2_IRQ; break;
                default: return -1;
            }
...
#if !USE_EXTINT
    ls2k_install_irq_handler(irqnum, ls2k_rtc_shared_isr, (void *)arg);
...
STATIC_DRV int RTC_initialize(const void *dev, void *arg)
{
    ...
    memset((void *)pRTC, 0, sizeof(rtc_priv_t));
