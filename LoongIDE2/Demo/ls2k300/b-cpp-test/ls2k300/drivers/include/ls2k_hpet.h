...
#if BSP_USE_HPET1
extern const void *devHPET1;
#endif

#if BSP_USE_HPET2
extern const void *devHPET2;
#endif

#if BSP_USE_HPET3
extern const void *devHPET3;
#endif

//-----------------------------------------------------------------------------
// 每个 HPET 有三个定时器子设备 ID
//-----------------------------------------------------------------------------
 ...
 */
typedef void (*hpetimer_callback_t)(const void *hpet, int timerID, int *stop);
...
{
    unsigned int timer;             /* HPET 定时器子设备: HPET_TIMERn */
    unsigned int work_mode;         /* 单次/周期, 边缘/电平 */

    /*
     * This parameter's unit is nanosecond(ns). After interrupt ocurred,
     * this value will auto loaded for next match interrupt.
     */
    unsigned long interval_ns;      /* 纳秒为单位进行定时 */

    irq_handler_t isr;              /* 优先级: 高. 安装外部独立中断 */
