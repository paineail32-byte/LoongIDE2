 ...
 * 测试使用 INT 接收、DMA 发送
 */
#define CONSOLE_DMA_INT     0       /* XXX 有影响? 发送使用 DMA, 是否造成 printk 重入? */
...
#if CONSOLE_DMA_INT
    ls2k_uart_ioctl(ConsolePort, IOCTL_UART_SET_RXTX_MODE, (void *)UART_TX_DMA);
#endif
}
...
#if CONSOLE_DMA_INT
    ls2k_uart_read(ConsolePort, &ch, 1, 0);
#else
...
#if CONSOLE_DMA_INT
    ls2k_uart_write(ConsolePort, &ch, 1, 0);
#else
...
#if CONSOLE_DMA_INT
    ls2k_uart_write(ConsolePort, buf, nbytes, 0);
