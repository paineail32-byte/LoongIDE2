...
#define MFRC522_BAUDRATE        400000
...
static int PCD_WriteRegister(const void *bus, unsigned char reg, unsigned char value)
{
	...
	rt = ls2k_i2c_write_bytes(bus, dataBuf, 2);	            /* write 2 bytes */
	rt = rt == 2 ? 0 : -1;
...
static int PCD_WriteRegisters(const void *bus, unsigned char reg, unsigned char count, unsigned char *values)
{
    ...
    unsigned char dataBuf[258];     // count 是 unsigned char 类型, 最大 256
    
    dataBuf[0] = reg;
    memcpy(&dataBuf[1], values, count);
	rt = ls2k_i2c_write_bytes(bus, dataBuf, (int)count+1);	    /* write count+1 bytes of values */
	rt = (rt == (int)count + 1) ? 0 : -1;
...
/*static */int PCD_Init(const void *bus)
{
    ...
    gpio_enable(RC522_RESET_PIN, DIR_OUT);     // reset pin
    gpio_enable(RC522_IRQ_PIN, DIR_IN);        // irq pin
