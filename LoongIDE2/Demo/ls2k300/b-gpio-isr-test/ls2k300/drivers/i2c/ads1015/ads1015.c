...
#define ADS1015_BAUDRATE        400000
 ...
 */
static int ADS1015_reg_access(const void *bus, uint8_t regNum, uint16_t *regVal, bool rw)
{
    int rt, rw_cnt;
	...
	/* start transfer */
	rt = ls2k_i2c_send_start(bus, ADS1015_ADDRESS);
	CHECK_DONE(rt);

	/* set transfer mode */
	rt = ls2k_i2c_ioctl(bus, IOCTL_SPI_I2C_SET_TFRMODE, (void *)ADS1015_BAUDRATE);
	CHECK_DONE(rt);

	/* address device, FALSE = WRITE */
	rt = ls2k_i2c_send_addr(bus, ADS1015_ADDRESS, false);
	CHECK_DONE(rt);
	 ...
	 */
	buf[0] = regNum;
	rw_cnt = ls2k_i2c_write_bytes(bus, buf, 1);
	if (rw_cnt < 0)
		rt = rw_cnt;
	CHECK_DONE(rt);
	...
	if (rw)		/* read */
	{
		 ...
		 */
		rt = ls2k_i2c_send_addr(bus, ADS1015_ADDRESS, true);
		CHECK_DONE(rt);

		/*
		 * fetch register data
		 */
	    rw_cnt = ls2k_i2c_read_bytes(bus, buf+1, 2);
        if (rw_cnt < 0)
		    rt = rw_cnt;
		CHECK_DONE(rt);
	...
	else		/* write */
	{
		...
		rt = ls2k_i2c_write_bytes(bus, buf+1, 2);
	}

lbl_done:
	/* terminate transfer */
	ls2k_i2c_send_stop(bus, ADS1015_ADDRESS);
 ...
 */
STATIC_DRV int ADS1015_read(const void *bus, void *buf, int size, void *arg)
{
    int rt = 0, channel = (int)((long)arg);
...
STATIC_DRV int ADS1015_ioctl(const void *bus, int cmd, void *arg)
{
	int rt = 0;
	uint16_t val = 0;
	...
	switch (cmd)
	{
		...
		case IOCTL_ADS1015_SET_CONV_CTRL:
			val = (uint16_t)((uintptr_t)arg);
			rt = ADS1015_reg_access(bus, ADS1015_REG_POINTER_CONFIG, &val, false);
			saved_config_reg = val;
			break;

		case IOCTL_ADS1015_GET_CONV_CTRL:
			rt = ADS1015_reg_access(bus, ADS1015_REG_POINTER_CONFIG, &val, true);
			*(uint16_t *)arg = val;
            saved_config_reg = val;
			break;
		...
		case IOCTL_ADS1015_SET_LOW_THRESH:
			val = (uint16_t)((uintptr_t)arg);
			rt = ADS1015_reg_access(bus, ADS1015_REG_POINTER_LOWTHRESH, &val, false);
			break;

		case IOCTL_ADS1015_GET_LOW_THRESH:
			rt = ADS1015_reg_access(bus, ADS1015_REG_POINTER_LOWTHRESH, &val, true);
			*(uint16_t *)arg = val;
			break;
		...
		case IOCTL_ADS1015_SET_HIGH_THRESH:
			val = (uint16_t)((uintptr_t)arg);
			rt = ADS1015_reg_access(bus, ADS1015_REG_POINTER_HITHRESH, &val, false);
			break;

		case IOCTL_ADS1015_GET_HIGH_THRESH:
			rt = ADS1015_reg_access(bus, ADS1015_REG_POINTER_HITHRESH, &val, true);
			*(uint16_t *)arg = val;
			break;
 ...
 */
static const driver_ops_t ADS1015_drv_ops =
{
    .init_entry  = NULL,
    .open_entry  = NULL,
    .close_entry = NULL,
...
const driver_ops_t *ads1015_drv_ops = &ADS1015_drv_ops;
#endif

/******************************************************************************
 * user api
 */
uint16_t get_ads1015_adc(const void *bus, int channel)
{
    if (ADS1015_ioctl(bus,
                      IOCTL_ADS1015_SET_CONV_CTRL,
                      (void *)((long)channel | ADS1015_CFG_DEFAULT)) >= 0)
    {
        uint16_t val = 0;
        osal_msleep(1);
