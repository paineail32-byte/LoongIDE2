 ...
 * 3. CAPTURE 脉冲测量
 *
 * 4. 芯片的PWM引脚可能被其它功能复用, 这时PWM设备仅可以用作内部定时器(脉冲信号不能从引脚输出).
 *
 ****************************************************************************************/
...
#define PWM_CAPTURE         0x10    // pulse capture
...
{
    unsigned int hi_ns;             /* high level nano seconds,  定时器模式仅用 hi_ns */ /* 测量模式用作超时 */
 ...
 * PWM 脉冲测量
 * 参数:    dev     devPWM0~devPWM3
 *          hi_ns   类型: unsigned int *, 返回测量值: 高电平纳秒数
 *          lo_ns   类型: unsigned int *, 返回测量值: 低电平纳秒数
 *          timeout_ms 超时毫秒数
 *
 * 返回:    0=成功
 *
