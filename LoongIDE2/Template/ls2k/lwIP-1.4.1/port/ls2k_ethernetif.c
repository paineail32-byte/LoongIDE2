...
#define netifINTERFACE_TASK_STACK_SIZE      DEFAULT_THREAD_STACKSIZE
...
static char   ls2k_gmac0_mac_addr[6] = { 0x72, 0x74, 0x00, 0x83, 0xDE, 0x12 };
static struct ethernetif ls2k_gmac0_ethernetif =
{
    .ethaddr  = NULL,
    .dev_gmac = NULL,
};

static struct netif ls2k_gmac0_netif;

struct netif *p_gmac0_netif = &ls2k_gmac0_netif;
...
static char   ls2k_gmac1_mac_addr[6] = { 0x72, 0x74, 0x00, 0x83, 0xEE, 0x12 };
static struct ethernetif ls2k_gmac1_ethernetif =
{
    .ethaddr  = NULL,
    .dev_gmac = NULL,
};

static struct netif ls2k_gmac1_netif;

struct netif *p_gmac1_netif = &ls2k_gmac1_netif;
...
#if BSP_USE_GMAC0
    if (pMAC == devGMAC0)
        p_mac_addr = ls2k_gmac0_mac_addr;
#endif

#if BSP_USE_GMAC1
    if (pMAC == devGMAC1)
        p_mac_addr = ls2k_gmac1_mac_addr;
#endif
...
#ifdef OS_FREERTOS
    /*
     * XXX Timer 中断在调用 sys_sem_new 的时候被关掉
     */
	// portENABLE_INTERRUPTS();		@@ FIXME @ 2025.2.1
#endif

	ls2k_gmac_init(pMAC, (void *)p_mac_addr);
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    if (ls2k_gmac_wait_tx_idle(pMAC, &p_txbuf) <= 0)
    {
     ...
     */
    ls2k_gmac_write(pMAC, (void *)p_txbuf, (int)p->tot_len, NULL);
...
static struct pbuf *low_level_input(struct netif *netif)
{
     ...
     * 取接收缓冲区
     */
    if (ls2k_gmac_wait_rx_packet(pMAC, &p_rxbuf) <= 0)
    {
        RW_UNLOCK(&ptr_if->rx_sem);
        return NULL;
    }

    len = ls2k_gmac_read(pMAC, (void *)p_rxbuf, 1514, NULL);
    if (len <= 0)
    {
        RW_UNLOCK(&ptr_if->rx_sem);
	    return NULL;
...
void ethernetif_input(void *pParams)
{
    ...
    extern void osal_msleep(uint32_t ms);
    osal_msleep(10);       /* 在其它任务后启动 */
    
    /**************************************************************************
     * 在这个位置启动 GMAC
     */
    if (ls2k_gmac_ioctl(pMAC, IOCTL_GMAC_IS_RUNNING, NULL) <= 0)
    {
        ls2k_gmac_ioctl(pMAC, IOCTL_GMAC_START, NULL);
    }
...
#endif
    {
         ...
         * 等待 GMAC_RX_EVENT
         */
        ls2k_gmac_wait_rx_packet(pMAC, &p_rxbuf);
...
void ls2k_initialize_lwip(unsigned char *ip0, unsigned char *ip1)
{
    int added_default = 0;
    ...
    ls2k_gmac0_ethernetif.dev_gmac = devGMAC0;      /* user data */

    /* add the network interface */
    netif_add(&ls2k_gmac0_netif,
              &ipaddr,
              &netmask,
              &gateway,
              &ls2k_gmac0_ethernetif,
              &ethernetif_init,
              &ethernet_input);
     ...
     */
    netif_set_default(&ls2k_gmac0_netif);
    netif_set_up(&ls2k_gmac0_netif);

    ls2k_gmac_ioctl(devGMAC0, IOCTL_GMAC_START, NULL);
    ...
    ls2k_gmac1_ethernetif.dev_gmac = devGMAC1;      /* user data */

    /* add the network interface */
    netif_add(&ls2k_gmac1_netif,
              &ipaddr,
              &netmask,
              &gateway,
              &ls2k_gmac1_ethernetif,
              &ethernetif_init,
              &ethernet_input);
     ...
     * Registers the default network interface
     */
    netif_set_default(&ls2k_gmac1_netif);
    netif_set_up(&ls2k_gmac1_netif);

    ls2k_gmac_ioctl(devGMAC1, IOCTL_GMAC_START, NULL);
