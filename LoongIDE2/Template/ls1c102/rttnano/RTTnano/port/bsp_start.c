...
void rt_hw_board_init(void)     // void bsp_start(void)
{
    ...
    rt_memcpy((void *)eentry, (void *)exception_common_entry, 32);

    g_pmu->CommandW = 0;            // CMDSR_SLEEP_EN;

#if USER_OUTER_OSC8M
    OSC_init_outer();               // 设置为外部时钟
#else
    ...
    /**
     * 此处需要关闭一些设备中断, 然后再开主中断: 防止开主中断时有设备中断产生
     *
     * 比如定时器中断
     */
    WRITE_REG32(LS1C102_TIMER_BASE, 0);
