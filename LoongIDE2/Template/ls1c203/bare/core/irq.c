...
ISR_Table_t __ISR_tbl[LS1C203_IRQ_COUNT];    // 在内存中
...
__WEAK void __ISR_dispatch(int vector, unsigned int *stack)
{
    ...
    {
        isr = __ISR_tbl[vector].isr;
        arg = __ISR_tbl[vector].arg;
...
__WEAK int ls1c203_install_isr(int vector, irq_handler_t isr, void *arg)
{
    ...
    {
        if (isr)
            __ISR_tbl[vector].isr = isr;
        else
            __ISR_tbl[vector].isr = __ISR_default;

        __ISR_tbl[vector].arg = arg;
...
__WEAK int ls1c203_remove_isr(int vector)
{
    if ((vector >= 0) && (vector < LS1C203_IRQ_COUNT))
    {
        __ISR_tbl[vector].isr = __ISR_default;
        __ISR_tbl[vector].arg = NULL;
...
__WEAK int ls1c203_interrupt_enable(int vector)
{
    switch (vector)
    {
...
__WEAK int ls1c203_interrupt_disable(int vector)
{
    switch (vector)
    {
