 ...
 * 需要二次分发的GPIO中断向量表
 */
static struct rt_irq_desc gpio_isr_table[GPIO_COUNT];
...
void loongarch_init_isr_table(void)
{
    ...
    {
		isr_table[i].handler = loongarch_default_isr;
		isr_table[i].param = 0;
	}

    for (i=0; i<GPIO_COUNT; i++)
    {
		gpio_isr_table[i].handler = loongarch_default_isr;
		gpio_isr_table[i].param = 0;
	}
...
static void bsp_irq_handler_dispatch(int vector, void *arg)
{
	if ((vector >= 0) && (vector < BSP_INTERRUPT_VECTOR_MAX))
	{
        ...
        {
            if (0 != isr_table[vector].param)
                isr_table[vector].handler(vector, (void *)isr_table[vector].param);
            else
                isr_table[vector].handler(vector, arg);   // 传递 stack 作为参数
        }
        else
        {
...
#if USE_EXTINT
__attribute__((weak)) void ls2k300_irq_dispatch_IP2(void (*dispatch)(int, void*), uint64_t *stack)
{
    return;
}
#endif
...
void ls2k_remove_irq_handler(int vector)
{
    if ((vector >= 0) && (vector < BSP_INTERRUPT_VECTOR_MAX))
    {
         ...
         * remove isr
         */
        isr_table[vector].handler = loongarch_default_isr;
        isr_table[vector].param = (void *)(uint64_t)vector;
...
static void ls2k300_gpio_common_isr(int vector, void *arg)
{
	...
	for (i=gpio1; i<=gpio2; i++)
	{
            ...
            READ_REG8(GPIO_ISR_ADDR + i))       	// Int Status
		{
			...
			if (gpio_isr_table[i].handler)
			{
				if (gpio_isr_table[i].param)
				{
					gpio_isr_table[i].handler(i, (void *)gpio_isr_table[i].param);
				}
				else
				{
					gpio_isr_table[i].handler(i, arg);  // 传递 arg 作为参数
				}
			}
			else
			{
...
int ls2k300_gpio_isr_install(int gpionum, void (*isr)(int, void *), void *arg)
{
    if ((gpionum >= 0) && (gpionum < GPIO_COUNT))
    {
         ...
         * install interrupt handler
         */
        gpio_isr_table[gpionum].handler = isr;
        gpio_isr_table[gpionum].param   = arg;
...
int ls2k300_gpio_isr_remove(int gpionum)
{
    if ((gpionum >= 0) && (gpionum < GPIO_COUNT))
    {
         ...
         * remove isr
         */
        gpio_isr_table[gpionum].handler = loongarch_default_isr;
        gpio_isr_table[gpionum].param = (void *)(long)gpionum;
                                         ...
                                         void *param, const char *name)
{
    ...
    if (vector >= 0 && vector < BSP_INTERRUPT_VECTOR_MAX)
    {
        ...
        ls2k_install_irq_handler(vector, handler, param);
    }
