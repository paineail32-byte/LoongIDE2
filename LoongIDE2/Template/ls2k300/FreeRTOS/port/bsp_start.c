...
void bsp_start(void)
{
    ...
    /**
     * 初始化内存管理: 使用 FreeRTOS 内存管理
     */
    #if 1
    {
        extern unsigned char _end;
        extern void xFreeRTOSInitialiseHeap_5(void *addr, size_t size);
        ...
        {
            xFreeRTOSInitialiseHeap_5((void *)heap_start, heap_end - heap_start);
        }
        else
        {
            for ( ; ; );
