                          ...
                          unsigned int refc)
{
    ...
    out = (div << PIX0_DIV_SHIFT) |
          (loopc << PIX0_DIV_LOOPC_SHIFT) |
          (refc << PIX0_DIV_REFC_SHIFT);

_again:

	WRITE_REG32(pix_pll_base, PIX0_PD_PLL);
	WRITE_REG32(pix_pll_base, PIX0_PD_PLL);
	WRITE_REG32(pix_pll_base, PIX0_PD_PLL);

	OR_REG32(pix_pll_base, out);
    AND_REG32(pix_pll_base, ~PIX0_PD_PLL);
    OR_REG32(pix_pll_base, PIX0_PLL_SOFT_SET);

	while (!(READ_REG32(pix_pll_base) & PIX0_PLL_LOCKED))
    {
        if (tmo++ > 100)
        {
    ...
    OR_REG32(pix_pll_base, PIX0_PLL_SEL);
    delay_us(1);
                             ...
                             unsigned int *p_refc)
{
    ...
    *p_refc = 0;
    ...
    for (refc = 1; refc <= 0x3f; refc++)
    {
         ...
         */
        calc_hz1 = osc_freq / refc;
        if ((calc_hz1 < 20000000) || (calc_hz1 > 40000000))
        {
            continue;
        }

        for (loopc = 1; loopc <= 0xFF; loopc++)
        {
...
static int ls2k_dc_hw_initialize(DC_softc_t *dc)
{
	...
	for (i=0; i<vgamode_count; i++)
	{
        ...
        {
            config_pix_pll(PLL_PIX0_BASE, div, loopc, refc);
        #if 0
            config_pix_pll(PLL_PIX1_BASE, div, loopc, refc);
        #endif
		    break;
	    }
...
STATIC_DRV int DC_initialize(const void *dev, void *arg)
{
     ...
     */
    pDC->hwDC = (HW_DC_t *)PHYS_TO_UNCACHED(DC0_BASE);
