...
#if USE_EXTINT
    .irqNum      = EXTINTC3_GMAC0_IRQ,
#else
    .irqNum      = INTC0_GMAC0_IRQ,
#endif
    .unitNumber  = 0,
...
#if USE_EXTINT
    .irqNum      = EXTINTC3_GMAC1_IRQ,
#else
    .irqNum      = INTC0_GMAC1_IRQ,
#endif
    .unitNumber  = 1,
	.descmode    = CHAINMODE,       // XXX This is important
    .timeout     = 0,
...
static void ls2k_gmac_set_start(GMAC_t *pMAC)
{
    if (!pMAC->started)
    {
        ...
        reg = READ_REG32(0x1fe1143c);

        if (pMAC == devGMAC0)
        {
            reg &= ~(1 << 12);
            WRITE_REG32(0x1fe1143c, reg);

            WRITE_REG32(0x1fe11428, (1 << 12));
        }
        else // if (pMAC == devGMAC0)
        {
            reg &= ~(1 << 14);
            WRITE_REG32(0x1fe1143c, reg);
...
static void ls2k_gmac_set_stop(GMAC_t *pMAC)
{
    if (pMAC->started)
    {
    	...
    	if (pMAC == devGMAC0)
    	{
    		WRITE_REG32(0x1fe1142c, (1 << 12));         /* INTC0 Clear */
    	}
    	else // if (pMAC == devGMAC1)
    	{
    		WRITE_REG32(0x1fe1142c, (1 << 14));
...
STATIC_DRV int GMAC_initialize(const void *dev, void *arg)
{
    ...
    #if !USE_EXTINT
        ls2k_set_irq_routeip(pMAC->irqNum, INT_ROUTE_IP3);
	#else
        WRITE_REG8(0x1fe1140c, 0x80);           /* Route to HW-IP5 */
    #endif
