 ...
 * 需要二次分发的GPIO中断向量表
 */
static struct rt_irq_desc gpio_isr_table[GPIO_INT_COUNT];
...
static void ls2k500_gpio_shared_irq_handler(int vector, void *arg)
{
    ...
    while (changed)
    {
        ...
        if (changed & 1)
        {
            ...
            if (gpio_isr_table[gpionum].handler)
            {
                if (0 != gpio_isr_table[gpionum].param)
                {
                    gpio_isr_table[gpionum].handler(gpionum, (void *)gpio_isr_table[gpionum].param);
                }
                else
                {
                    gpio_isr_table[gpionum].handler(gpionum, arg);  // 传递 arg 作为参数
                }
            }
            else
            {
...
int ls2k500_gpio_isr_remove(int gpionum)
{
    if ((gpionum >= 0) && (gpionum < GPIO_INT_COUNT))
    {
        ...
        gpio_isr_table[gpionum].param = (void *)(uint64_t)gpionum;
