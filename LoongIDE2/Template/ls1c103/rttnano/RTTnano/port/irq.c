 ...
 * 中断向量表
 */
static struct rt_irq_desc isr_table[LS1C103_IRQ_COUNT];
...
void ls1c103_init_isr_table(void)
{
    ...
    {
		isr_table[i].handler = ls1c103_default_isr;
		isr_table[i].param = 0;
    }
}
...
static void ls1c103_dispatch_isr(int vector, unsigned int *stack)
{
    ...
    {
        ls1c103_default_isr(vector, stack);
    }
}
...
int ls1c103_install_isr(int vector, irq_handler_t isr, void *arg)
{
    if ((vector >= 0) && (vector < LS1C103_IRQ_COUNT))
    {
        if (isr)
            ...
            isr_table[vector].handler = ls1c103_default_isr;
...
{
    ls1c103_interrupt_disable(vector);
}

void rt_hw_interrupt_umask(int vector)
{
    ls1c103_interrupt_enable(vector);
}
                                         ...
                                         void *param, const char *name)
{
    ...
    if (vector >= 0 && vector < LS1C103_IRQ_COUNT)
    {
        ...
        ls1c103_install_isr(vector, handler, param);
    }
