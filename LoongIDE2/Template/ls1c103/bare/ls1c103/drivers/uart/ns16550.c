 ...
 * NS16550 Interrupt.
 */
static void ls1c103_enable_uart_irq(unsigned int ns16550_base)
{
    if (ns16550_base == LS1C103_UART0_BASE)
    {
        OR_REG32(INTC_EN_BASE, INTC_UART0_BIT);
    }
    else if (ns16550_base == LS1C103_UART1_BASE)
    {
        OR_REG32(INTC_EN_BASE, INTC_UART1_BIT);
    }
}

static void ls1c103_disable_uart_irq(unsigned int ns16550_base)
{
    if (ns16550_base == LS1C103_UART0_BASE)
    {
        AND_REG32(INTC_EN_BASE, ~INTC_UART0_BIT);
    }
    else if (ns16550_base == LS1C103_UART1_BASE)
    {
        AND_REG32(INTC_EN_BASE, ~INTC_UART1_BIT);
    }
}

static void ls1c103_clear_uart_irq(unsigned int ns16550_base)
{
    if (ns16550_base == LS1C103_UART0_BASE)
    {
...
static void NS16550_interrupt_handler(int vector, void *arg)
{
    ...
    if (NULL != pUART)
    {
        ls1c103_disable_uart_irq((unsigned int)pUART->hwUART);  /* 关中断 */
        ls1c103_clear_uart_irq((unsigned int)pUART->hwUART);    /* 清中断 */

        NS16550_interrupt_process(pUART);                       /* 中断处理 */

        ls1c103_enable_uart_irq((unsigned int)pUART->hwUART);   /* 开中断 */
    }
}
...
int NS16550_init(void *dev, void *arg)
{
    ...
    {
        /*
         * PB00 <--> GPIO16 : TX 配置成推挽输出模式
         */
        ls1c103_gpio_cfg(16, OUT_PULL, IO_OUT);

        /*
         * PB01 <--> GPIO17 : RX 配置成浮空输入模式
         */
        ls1c103_gpio_cfg(17, IN_FLOAT, IO_IN);

        /*
         * PB00 <--> GPIO16 : 第一复用 UART1_TX
         * PB01 <--> GPIO17 : 第一复用 UART1_RX
         */
        ls1c103_iosel(16, IOSEL_MUX1);
        ls1c103_iosel(17, IOSEL_MUX1);
    }
#endif
    else
    {
...
int NS16550_open(void *dev, void *arg)
{
    ...
    if (pUART->bInterrupt)
    {
         ...
         * 安装中断向量
         */
        ls1c103_install_isr(pUART->irqNum,
...
int NS16550_close(void *dev, void *arg)
{
    ...
    if (pUART->bInterrupt)
    {
        ls1c103_disable_uart_irq((unsigned int)pUART->hwUART);
        
        ls1c103_remove_isr(pUART->irqNum);          /* 移除中断向量 */
    }
...
{
    .hwUART     = (HW_NS16550_t *)LS1C103_UART0_BASE,
    .irqNum     = LS1C103_IRQ_UART0,
    .bInterrupt = false,
    .baudRate   = 115200,
};
...
{
    .hwUART     = (HW_NS16550_t *)LS1C103_UART1_BASE,
    .irqNum     = LS1C103_IRQ_UART1,
    .bInterrupt = false,
    .baudRate   = 115200,
};
