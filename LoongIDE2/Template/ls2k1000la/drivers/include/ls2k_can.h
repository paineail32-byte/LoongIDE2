 ...
 *      Then Start It:
 *
 *      ls2k_can_ioctl(devCAN0, IOCTL_CAN_START, NULL);
 *
 * Read:
 *
 *      int readed;
 ...
 *      msg.id=x;
 *      ...
 *      readed = ls2k_can_read(devCAN0, &msg, sizeof(msg), NULL);
 ...
 *      msg.id=x;
 *      ...
 *      writted = ls2k_can_write(devCAN0, &msg, sizeof(msg), NULL);
...
/*
 * int ls2k_can_register(amba_confarea_type *bus);
*/
 ...
 *          通信速率: CAN_SPEED_250K
 */
int CAN_init(const void *dev, void *arg);
 ...
 *      ---------------------------------------------------------------------------------
 *          IOCTL_CAN_SET_SPEED         |   类型: unsigned long
 *                                      |   用途: 设置CAN设备的通信速率, 见"CAN_SPEED_x"定义
 *      ---------------------------------------------------------------------------------
 *          IOCTL_CAN_SET_FILTER        |   类型: CAN_afilter_t *
 *                                      |   用途: 设置CAN设备的硬件过滤器
 *      ---------------------------------------------------------------------------------
 *          IOCTL_CAN_SET_BUFLEN        |   类型: unsigned long
 *                                      |   用途: 更改CAN设备的内部缓存大小.
 *                                      |         低16位: 接收缓冲区个数;
 *                                      |         高16位: 发送缓冲区个数.
 *      ---------------------------------------------------------------------------------
 *          IOCTL_CAN_SET_CORE          |   类型: unsigned long
 *                                      |   用途: 设置CAN设备的内核模式,
 *                                      |         CAN_CORE_20A/CAN_CORE_20B
 *      ---------------------------------------------------------------------------------
 *          IOCTL_CAN_SET_WORKMODE      |   类型: unsigned long
 *                                      |   用途: 设置CAN设备2.0B的工作模式,
 *                                      |         CAN_STAND_MODE/CAN_SLEEP_MODE/
 *                                      |         CAN_LISTEN_ONLY/CAN_SELF_RECEIVE
 *      ---------------------------------------------------------------------------------
 *          IOCTL_CAN_SET_TIMEOUT       |   类型: unsigned long
