 ...
 */
    ...
    { 25000, 60,  480,  488,  490,  498,  800,  817,  819,  832, 0x00000301, 0xc0000000}, /*"480x800_60.00"*/ /* ST7701s */
...
static int config_pix0_pll(unsigned long l1_ref, unsigned long l1_loopc, unsigned long l2_div)
{
    int tmo = 0;

_again:

	WRITE_REG64(PIX0_PLL0_BASE, PIX0_PLL0_L1_PD);
	WRITE_REG64(PIX0_PLL0_BASE, PIX0_PLL0_L1_PD);
	WRITE_REG64(PIX0_PLL0_BASE, PIX0_PLL0_L1_PD);

	OR_REG64(PIX0_PLL0_BASE, ((l1_loopc << PIX0_PLL0_L1_DIV_LOOPC_SHIFT) & PIX0_PLL0_L1_DIV_LOOPC_MASK) |
                             ((l1_ref   << PIX0_PLL0_L1_DIV_REF_SHIFT  ) & PIX0_PLL0_L1_DIV_REF_MASK  ));
	WRITE_REG64(PIX0_PLL1_BASE, l2_div & PIX0_PLL1_L2_DIV_OUT_MASK);

	AND_REG64(PIX0_PLL0_BASE, ~PIX0_PLL0_L1_PD);
	OR_REG64(PIX0_PLL0_BASE, PIX0_PLL0_SOFT_SET);
	
	while (!(READ_REG64(PIX0_PLL0_BASE) & PIX0_PLL0_L1_LOCKED))
    { 
        if (tmo++ > 100)
        {
    ...
    OR_REG64(PIX0_PLL0_BASE, PIX0_PLL0_SEL_PIX0);
    delay_us(1);
...
static unsigned int cal_freq(unsigned int pix_hz,
                             unsigned int *p_l1_ref,
                             unsigned int *p_l1_loopc,
                             unsigned int *p_l2_div)
{
	unsigned int l1_ref, l1_loopc, l2_div;
	unsigned long bias, bias_min = 1000;
    unsigned long osc_freq = 100000000;     // 100M

    *p_l1_ref   = 0;
    *p_l1_loopc = 0;
    *p_l2_div   = 0;
                             
    for (l1_ref = 1; l1_ref <= 0x3F; l1_ref++)
    {
         ...
         */
        calc_hz1 = osc_freq / l1_ref;
        if ((calc_hz1 < 20000000) || (calc_hz1 > 40000000))
        {
            continue;
        }

        for (l1_loopc = 1; l1_loopc <= 0x3ff; l1_loopc++)
        {
            unsigned long calc_hz2;
            
            /*
             * PLL 倍频值 refclk/div_ref*div_loopc 需要在 1.2GHz~3.2GHz
             */
            calc_hz2 = calc_hz1 * l1_loopc;
            if ((calc_hz2 < 1200000000) || (calc_hz2 > 3200000000))
            {
                continue;
            }
            ...
            for (l2_div = 1; l2_div <= 0x3f; l2_div++)
            {
                unsigned long calc_hz3;

                calc_hz3 = calc_hz2 / l2_div;
...
static int ls2k_dc_hw_initialize(DC_softc_t *dc)
{
	...
	for (i=0; i<vgamode_count; i++)
	{
        ...
        if (cal_freq(vga_modes[i].pclk, &l1_ref, &l1_loopc, &l2_div) == 0)
        {
            config_pix0_pll((unsigned long)l1_ref, (unsigned long)l1_loopc, (unsigned long)l2_div);

        #if 0
            config_pix1_pll((unsigned long)l1_ref, (unsigned long)l1_loopc, (unsigned long)l2_div);
        #endif
...
STATIC_DRV int DC_initialize(const void *dev, void *arg)
{
    ...
    /*
     * FB0 as framebuffer controller
     */
    dc_base = READ_REG64(DC_CFG_HEAD_BASE + 0x10) & ~0x0Ful;
    pDC->hwDC = (HW_DC_t *)PHYS_TO_UNCACHED(dc_base);

    ls2k_dc_init_hook(dev);
