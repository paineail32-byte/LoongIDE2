/*
 *  LS2K CAN driver interface
 *
 *  Author: Bian, 2023/10/16
 */

#ifndef __LS2K_CAN_H__
#define __LS2K_CAN_H__
...
#define LS2K_DEVNM_CAN0		"/dev/can0"
#define LS2K_DEVNM_CAN1		"/dev/can1"
#ifdef LS2K500
#define LS2K_DEVNM_CAN2		"/dev/can2"
#define LS2K_DEVNM_CAN3		"/dev/can3"
#endif
 ...
 *
 * Start: XXX this need before rw.
 *
 *		ioctl(fd, IOCTL_CAN_START);
 *
 * Read:
 *
 * 		CANMsg_t msg; int readed;
 * 		msg.id=x;
 * 		...
 * 		readed = read(fd, &msg, sizeof(msg));
 ...
 * Write:
 *
 * 		CANMsg_t msg; int writted;
 * 		msg.id=x;
 * 		...
 * 		writted = write(fd, &msg, sizeof(msg));
...
/*
 * install all can drivers
 */
rtems_status_code bsp_install_can_driver(void);

//-----------------------------------------------------------------------------
// CAN access parameter type
//-----------------------------------------------------------------------------
